...
export class WalletImport {
    ...
    const waitForElements = () => {
      ...
      input.addEventListener("input", (e) => {
        ...
        if (value) {
          ...
          if (inputGroup) {
            inputGroup.style.borderColor = isValid
              ? "var(--accent-green)"
              : "var(--accent-red)";
          }
  ...
  validateFile(file) {
    ...
    if (
      ...
      !file.name.match(/\.(json|csv|txt)$/i)
    ) {
      this.showError(
        "Invalid file type. Please upload JSON, CSV, or TXT files."
      );
      return false;
    }
  ...
  async processWallet(walletData) {
    ...
    try {
      ...
      const scoringCompleteEvent = new CustomEvent("scoringComplete", {
        detail: {
          ...
          scoreData: {
            score:
              result.score ||
              (result.creditScore ? result.creditScore.score : 720),
            scoreLabel:
              result.scoreLabel ||
              (result.creditScore
                ? result.creditScore.scoreLabel
                : this.getScoreLabel(result.score || 720)),
            explanation:
              result.explanation ||
              (result.creditScore ? result.creditScore.explanation : ""),
          },
        },
      });
      ...
      this.showError(
        "Processing failed: " +
          error.message +
          ". Please check that both API servers are running."
      );
  ...
  async callAPI(walletData) {
    try {
      ...
      this.showError(
        `API Error: ${error.message}. Please check your API servers.`
      );
